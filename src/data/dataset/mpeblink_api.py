...
class MPEblink:
    ...
    def createIndex(self):
        ...
        if 'annotations' in self.dataset:
            for ann in self.dataset['annotations']:
                ...
                anns[ann['id']] = ann
        # vidToAnns         video                                 list,            k                  instance         (len=10   dict)
        if 'videos' in self.dataset:
            for vid in self.dataset['videos']:
                vids[vid['id']] = vid
        # vids         video                        video                                                                     dict=9
        ...
        # 40                                                                  videod   id,   instance-level               instance                  
        print('index created!')
    ...
    def loadRes(self, resFile):
        ...
        """
        res = MPEblink()  #                      res               dict
        res.dataset['videos'] = [img for img in self.dataset[
            'videos']]  # 'videos'                              train.json                                 train.json            self.dataset                                 result.json                  

        print('Loading and preparing results...')
        tic = time.time()
        if type(resFile) == str or (PYTHON_VERSION == 2 and type(resFile) == unicode):
            anns = json.load(open(resFile))  #                            result,         *10 10                     top10   query
        ...
        for id, ann in enumerate(anns):  # ann                                 query,                     ann               anns                        
            ann['areas'] = []
            for bbox in ann['bboxes']:  #                                  seg
                # now only support compressed RLE format as segmentation results
                # ann['areas'].append(bbox[2]*bbox[3]) if bbox != None else ann['areas'].append(None)
                if bbox == None:  #                         json   gt               None
                    ann['areas'].append(0)
                else:
        ...
        res.dataset[
            'annotations'] = anns  #                   result.json                        train.json                              instance id                  query,               video   topk   query,      video         topk   id
        res.createIndex()
        return res